	...
	while (currentTime <= sumTime) {
		...
		while (true) {
			...
			if (push != arrivalTime.end()) {
				...
				arrivalTime[temp] = -1;
				process.push(make_pair(priority[temp], temp * -1));
			}
			else
				break;
		}
		...
		string ProcessInProcess = processName[pairr.second * -1];
		process.pop();
		...
		if (cpuBurst[indexProcess] == 0) {
			...
			process.push(make_pair(priority[indexProcess], indexProcess * -1));
		}
